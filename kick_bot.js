...
chatChannel.bind('App\\\\Events\\\\ChatMessageEvent', async (data) => {
        try {
            ...
            // (Manualni admin prikazy !zapnout a !vypnout byly odstraneny - bot bezi plne automaticky)
        } catch (err) {
...
// --- HTTP Server pro Render.com (health check + ping proti uspani) ---
const http = require('http');
const PORT = process.env.PORT || 10000;

http.createServer((req, res) => {
        // Zdravotni check a probouzeci endpoint pro Vercel cron
                      res.writeHead(200, { 'Content-Type': 'text/plain' });
        res.end(`Kick bot bezi plne automaticky | Stream: ${isStreamOnlineCache ? 'ONLINE' : 'OFFLINE'} | Aktivni divaci (zapsali do chatu): ${activeUsers.size}`);
}).listen(PORT, () => {
        console.log(`[WEB] Ping server bezi na portu ${PORT} - Bot nebude uspat.`);
});
